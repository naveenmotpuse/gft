var _Scenario = (function() {
  ...
  var scenario_q21 = [["radio1"], ["radio2"]];
  var calories_q21 = [[31.25, 56.25], [125, 100]];
  ...
  return {
    ...
    UpdateQuestionData: function() {
      ...
      _QData.Q18.graphData[1] =
        userScenario[scenarioIndex].ppfData[
          userScenario[scenarioIndex].ppfData.length - 1
        ];
      _QData.Q18.correctData = userScenario[scenarioIndex].ppfData;

      _QData.Q19.graphData[0] = fridayScenario[scenarioIndex].ppfData[0];
      _QData.Q19.graphData[1] =
        fridayScenario[scenarioIndex].ppfData[
          fridayScenario[scenarioIndex].ppfData.length - 1
        ];
    ...
    if ($(_this).find("img").length > 0) {
      if (
        $(_this)
          .find("img")
          .attr("src")
          .indexOf("down-chevron.png") > 0
      ) {
        $(_this)
          .find("img")
          .attr("src", "assets/images/up-chevron.png");
        $(_this)
        ...
        $(_this)
          .find("img")
          .attr("src", "assets/images/down-chevron.png");
        $(_this)
...
var _Template = (function() {
  return {
    ...
    LoadDaytimeScheduler: function() {
      var pageUrl =
        "templates/daytimescheduler.htm" + _Caching.GetUrlExtension();
      $(".daytime_scheduler").load(pageUrl, function() {});
    },
    LoadNighttimeScheduler: function() {
      var pageUrl =
        "templates/nighttimescheduler.htm" + _Caching.GetUrlExtension();
  ...
  return {
    ...
    OnQuestionLoad: function() {
      ...
      if (qObj.Id == "Q15") {
        $("#graph-div").attr(
          "aria-label",
          "Daytime Schedule Collect Wood 12 Hour Collect Fish 0 Hour Idle 0 Hour"
        );
        $("#img-stranded-island").attr(
          "alt",
          "Friday rafts over to your island proposing that you could be better off through trade."
        );
      }
      if (qObj.Id == "Q16") {
        $("#graph-div").attr(
      ...
      if (qObj.Id == "Q22") {
        $("#graph-div").attr(
          "aria-label",
          "Daytime Schedule Collect Wood 0 Hour Collect Fish 12 Hour Idle 0 Hour"
        );
        $("#img-stranded-island").attr(
          "alt",
          "Friday rafts over to your island proposing that you could be better off through trade."
        ...
        if (isNaN(fish) || isNaN(wood)) {
          ...
          if (series.data.length < valPoints) {
            ...
            for (var i = 0; i < series.length; i++) {
              ...
              var isonline = this.IsPointOnLine(currPoint, [
                Number(wood),
                Number(fish)
              ]);
              if (!isonline) {
    ...
    CheckGraphAnswer: function(valPoints) {
      ...
      if (allliesonline) {
        ...
        if (_currentQuestionObj.tryCount < _currentQuestionObj.totalTry) {
          ...
          var attemptCurrentQuestionData = _Navigator.GetQuestionAttemptData(
            pageId,
            Qid
          );
          if (attemptCurrentQuestionData != undefined) {
            ...
            for (
              var i = 0;
              i < attemptCurrentQuestionData.selectedAnswer.length;
              i++
            ) {
              if (
                newSerData[i][0] !=
                  attemptCurrentQuestionData.selectedAnswer[i][0] &&
                newSerData[i][1] !=
                  attemptCurrentQuestionData.selectedAnswer[i][1]
              ) {
                newSerData[newSerDataIndex] = [
                  attemptCurrentQuestionData.selectedAnswer[i][0],
                  attemptCurrentQuestionData.selectedAnswer[i][1]
                ];
                newSerDataIndex++;
            ...
            for (var i = 0; i < newSerData.length; i++) {
              ...
              if (!isonline) {
                ...
                crrcount++;
                _currentQuestionObj.selectedAnswer[countval] = [
                  newSerData[i][0],
                  newSerData[i][1]
                ];
                countval++;
          ...
          _currentQuestionObj.isAnswered = true;
          if (isWorsen) {
            _currentQuestionObj.feedbackIndex =
              attemptCurrentQuestionData.feedbackIndex;
          } else {
            _currentQuestionObj.feedbackIndex = feedbackIndex;
          }
    ...
    ActivityPrevAnswer: function() {
      ...
      if (currPage.hasTradeSlider != undefined && currPage.hasTradeSlider) {
        ...
        $("#givewood-range").attr(
          "max",
          activityData.tradeData.TR.consumptionwood +
            activityData.tradeData.TR.givewood
        );
        $("#givewood-range").val(activityData.tradeData.TR.givewood);
        $("#givewood-logs").text(activityData.tradeData.TR.givewood);
        $("#receivefish-cals").text(
          activityData.tradeData.TR.receivefish - activityData.remFish
        );

        $("#consumption-wood").text(activityData.tradeData.TR.consumptionwood);
        $(".consumption-wood.r_label").text(
          activityData.tradeData.TR.consumptionwood +
            activityData.tradeData.TR.givewood
        );
        $("#consumption-wood-range").attr(
          "max",
          activityData.tradeData.TR.consumptionwood +
            activityData.tradeData.TR.givewood
        );
        $("#consumption-wood-range").val(
          activityData.tradeData.TR.consumptionwood
        );
        $("#consumption-fish").text(activityData.tradeData.TR.consumptionfish);
        $("#consumption-fish-range").attr(
          "max",
          activityData.tradeData.TR.fridayconsumptionfish +
            activityData.tradeData.TR.receivefish
        );
        $("#consumption-fish-range").val(
          activityData.tradeData.TR.consumptionfish
        );
        $(".consumption-fish.r_label").text(
          activityData.tradeData.TR.fridayconsumptionfish +
            activityData.tradeData.TR.receivefish
        );
        ...
        var _wPerc =
          (Number($("#onewoodfor-range").val()) /
            Number($("#onewoodfor-range").attr("max"))) *
            95 +
          "%";
        $("#oneWoodFiller").css("width", _wPerc);
        _wPerc =
          (Number($("#givewood-range").val()) /
            Number($("#givewood-range").attr("max"))) *
            95 +
          "%";
        $("#giveWoodFiller").css("width", _wPerc);
        _wPerc =
          (Number($("#consumption-wood-range").val()) /
            Number($("#consumption-wood-range").attr("max"))) *
            95 +
          "%";
        $("#consumeWoodFiller").css("width", _wPerc);
        _wPerc =
          (Number($("#consumption-fish-range").val()) /
            Number($("#consumption-fish-range").attr("max"))) *
            95 +
          "%";
        $("#consumeFishFiller").css("width", _wPerc);

        _TradeSlider.ShowSliderPoint("studenttradeGraph", [
          activityData.tradeData.TR.consumptionwood,
          activityData.tradeData.TR.consumptionfish
        ]);
        _TradeSlider.ShowSliderPoint("fridaytradeGraph", [
          activityData.tradeData.TR.fridayconsumptionwood,
          activityData.tradeData.TR.fridayconsumptionfish
        ]);
...
var _CustomPage = (function() {
  return {
    OnPageLoad: function() {
      ...
      if (
        currPage.pageId == "l3p2" ||
        currPage.pageId == "l3p3" ||
        currPage.pageId == "l4p5"
      ) {
      } else {
      ...
      if (_currPageId != undefined) {
        ...
        if (isIOS) {
          $("#ppfchart_c").attr({
            role: "text",
            "aria-label":
              "Production Possibilities Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 7000 in calories. Refer table for more details."
          });
          $("#surpluschart_c").attr({
            role: "text",
            "aria-label":
              "Surplus Inventory graph for inventory of fish and woods from day 0 to 4. Goals: Wood: 90 (logs) - Fish: 9000 (cals)."
          });
          ...
          if (_currPageId == "l1p2" && _currPageQId == "Q3") {
            $("#questionchart").attr({
              role: "text",
              "aria-label":
                "Your Production Possibility Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 7000 in calories. Refer above table for more details."
            });
          }
          if (_currPageId == "l1p4" && _currPageQId == "Q7") {
            $("#questionIntro").attr({
              role: "text",
              "aria-label":
                "Friday's Production Possibility Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 6500 in calories. Refer above table for more details."
            });
          }
          if (
            _currPageId == "l2p2" &&
            ...
            $("#questionIntro").attr({
              role: "text",
              "aria-label":
                "Production Possibilities Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 7000 in calories. Graph represents your PPF, Friday's PPF and Both Student and Friday's Production Point."
            });
          }
          if (_currPageId == "l2p3" && _currPageQId == "Q15") {
            $("#studenttradeGraph").attr({
              role: "text",
              "aria-label":
                "Your Production Possibility graph for Firewoods from 0 to 110 in logs vs. Fish from 0 to 5000 in calories."
            });
            $("#fridaytradeGraph").attr({
              role: "text",
              "aria-label":
                "Friday's Production Possibility Frontier graph for Firewoods from 0 to 110 in logs vs. Fish from 0 to 8000 in calories."
            });
          }
          ...
          if (_currPageId == "l4p2" && _currPageQId == "Q18") {
            $("#questionchart").attr({
              role: "text",
              "aria-label":
                "Your Production Possibility Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 6500 in calories. Refer above table for more details."
            });
          }
          if (_currPageId == "l4p3" && _currPageQId == "Q19") {
            $("#questionchart").attr({
              role: "text",
              "aria-label":
                "Your Production Possibility Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 4500 in calories. Refer above table for more details."
            });
          }
          if (
            _currPageId == "l4p4" &&
            (_currPageQId == "Q20" || _currPageQId == "Q21")
          ) {
            $("#questionIntro").attr({
              role: "text",
              "aria-label":
                "Production Possibilities Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 7000 in calories. Graph represents your PPF, Friday's PPF."
            });
          }
          if (_currPageId == "l4p5" && _currPageQId == "Q22") {
            $("#studenttradeGraph").attr({
              role: "text",
              "aria-label":
                "Your Production Possibility graph for Firewoods from 0 to 110 in logs vs. Fish from 0 to 8000 in calories."
            });
            $("#fridaytradeGraph").attr({
              role: "text",
              "aria-label":
                "Friday's Production Possibility Frontier graph for Firewoods from 0 to 110 in logs vs. Fish from 0 to 5000 in calories."
            });
          }
        } else {
          if (_currentPage.datalevel == 0 || _currPageId == "l1p1") {
            ...
            _ModuleCharts.IEGraphAccessibility(
              "ppfchart_c",
              "Production Possibilities Frontier graph for Firewoods from 0 to 120 in logs vs. Fish from 0 to 7000 in calories. Refer table for more details."
            );
          }
          ...
          if (
            _currPageId == "l1p4" &&
            (_currPageQId == "Q3" || _currPageQId == "Q7")
          ) {
            $("#questionIntro")
              .find(".highcharts-container")
              .attr("aria-hidden", "true");
